        ...
        VStack(alignment: .leading, spacing: 18) {
                        ...
                        HStack(alignment: .top) {
                            ...
                            VStack(alignment: .leading, spacing: 3) {
                                ...
                                if destination.existingCount > destination.managedCount {
                                    Text("\(destination.existingCount - destination.managedCount) items are not accessible to Photo Curator and will be left untouched.")
            ...
            if review.canReplace && !hasUnresolved {
                Picker("Update this selection", selection: $replace) {
                    ...
                    Text("Replace Photo Curator photos").tag(true)
                }
                ...
                Text(replace
                     ...
                     : "Keep the current album contents and add this selection. Previously confirmed Photo Curator uploads are reused, not uploaded again.")
                    .font(.callout).foregroundStyle(.secondary)
            } else if !hasUnresolved {
                Text("No existing Photo Curator photos need replacing. Your selection will be added without removing anything.")
                    .font(.callout).foregroundStyle(.secondary)
            }

            if isFrame {
                Text("Your Nest Hub will pick up the updated album on its own schedule. Photo Curator cannot force the display to refresh.")
                    .font(.caption).foregroundStyle(.secondary)
            }
            Divider()
            HStack {
