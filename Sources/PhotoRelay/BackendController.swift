...
final class BackendController: ObservableObject {
    ...
    func start() throws {
        ...
        guard let executable = ProcessInfo.processInfo.environment["PHOTO_CURATOR_GOOGLE_HELPER"].map({ URL(fileURLWithPath: $0) })
                ?? Bundle.main.resourceURL?.appendingPathComponent("Engine/PhotoCuratorGoogleHelper") else { throw CocoaError(.fileNoSuchFile) }
