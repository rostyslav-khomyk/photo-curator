...
PACKAGE_DIR="$ROOT_DIR/macos/PhotoRelay"
...
PHOTO_CURATOR_NATIVE_BUILD=1 "$ROOT_DIR/.venv/bin/pyinstaller" --clean --noconfirm "$ROOT_DIR/icloudpd.spec"
...
cp "$ROOT_DIR/dist/icloudpd" "$APP_DIR/Contents/Resources/Engine/icloudpd"
cp "$PACKAGE_DIR/Info.plist" "$APP_DIR/Contents/Info.plist"
swift "$ROOT_DIR/macos/render_icon.swift" "$PACKAGE_DIR/.build/PhotoRelay.iconset"
...
chmod +x "$APP_DIR/Contents/MacOS/PhotoRelay" "$APP_DIR/Contents/Resources/Engine/icloudpd"
