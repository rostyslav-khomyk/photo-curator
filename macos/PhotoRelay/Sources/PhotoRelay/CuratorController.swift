...
final class CuratorController: ObservableObject {
    ...
    func runDiagnostic() {
        ...
        guard PHPhotoLibrary.authorizationStatus(for: .readWrite) == .authorized else {
            diagnosticReport = "Allow Photo Relay access in System Settings > Privacy & Security > Photos. Full Disk Access is not needed."
            return
        }
    ...
    func setLoginEnabled(_ enabled: Bool) {
        do {
            ...
            if enabled && !loginEnabled {
                errorMessage = "Allow Photo Relay in System Settings > General > Login Items."
            }
        ...
        analysisTask = Task(priority: .utility) {
            ...
            do {
                ...
                if let job = claimed {
                    ...
                    } catch { errorMessage = "Could not save analysis progress. Retry after reopening Photo Relay." }
                }
            }
        }
    }
