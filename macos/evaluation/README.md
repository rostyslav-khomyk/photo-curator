...
`swift macos/evaluation/render-selected-review.swift REPORT_FOLDER` to render the actual
...
`PHOTO_RELAY_VALIDATION_REPORT=REPORT_FOLDER swift test --package-path macos/PhotoRelay --filter ExportedReferenceBaselineTests/testOptInLargeVisitAndHoldoutValidation`
...
4. Run the app metadata baseline and local evidence pilot with
   `PHOTO_RELAY_REFERENCE_REPORT=REPORT_FOLDER swift test --package-path macos/PhotoRelay --filter ExportedReferenceBaselineTests`
...
Audit checks: `node --test macos/evaluation/audit-export.test.mjs` from repository root.
