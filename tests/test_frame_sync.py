...
from icloudpd.frame_sync import FrameSyncPlan, SyncAborted
...
def test_server_requires_review_and_csrf_and_exposes_progress(tmp_path, monkeypatch):
    captured = {}
    fake = FakeClient()
    control = WebControl(lambda _: 0, logging.getLogger("test"))
    monkeypatch.setattr(
        "icloudpd.google_photos_client.GooglePhotosClient", lambda *args, **kwargs: fake
    )
    monkeypatch.setattr(
        "icloudpd.server.waitress.serve", lambda app, **kwargs: captured.update(app=app)
    )
    serve_app(logging.getLogger("test"), StatusExchange(), control=control)
    client = captured["app"].test_client()
    headers = {"X-CSRF-Token": client.get("/auth-state").json["csrf_token"]}
    assert (
        client.post("/start-frame-sync", json={"token": "bad", "mode": "replace"}).status_code
        == 403
    )
    assert (
        client.post(
            "/start-frame-sync", json={"token": "bad", "mode": "replace"}, headers=headers
        ).status_code
        == 409
    )
    ...
    payload = {
        "credentials": str(tmp_path / "google.json"),
        "items": [{"path": str(photo), "album": "Trip"}],
        "destinations": {"Trip": {"id": "frame"}},
    }
    review = client.post("/prepare-frame-sync", json=payload, headers=headers)
    assert review.status_code == 200
    assert fake.events == []
    token = review.json["token"]
    assert (
        client.post(
            "/start-frame-sync", json={"token": token, "mode": "erase"}, headers=headers
        ).status_code
        == 409
    )
    assert (
        client.post(
            "/start-frame-sync", json={"token": token, "mode": "append"}, headers=headers
        ).status_code
        == 202
    )
    for _ in range(100):
        state = client.get("/control-state").json
        if not state["running"]:
            break
        time.sleep(0.01)
    assert state["last_exit_code"] == 0
    assert state["progress"]["run_id"] == token
    assert state["progress"]["phase"] == "complete"
    assert (
        client.post(
            "/start-frame-sync", json={"token": token, "mode": "append"}, headers=headers
        ).status_code
...
def test_sync_cannot_start_while_review_is_busy():
    control = WebControl(lambda _: 0, logging.getLogger("test"))
    control._browser_lock.acquire()
    try:
        accepted, message = control.start([])
        assert not accepted
        assert "review" in message
    finally:
        control._browser_lock.release()
...
def test_eta_is_transfer_only_and_appears_after_enough_samples(tmp_path, monkeypatch):
    client = FakeClient()
    clock = [100.0]
    monkeypatch.setattr("icloudpd.frame_sync.time.monotonic", lambda: clock[0])
...
def test_abort_endpoint_is_csrf_protected_and_bound_to_current_run(monkeypatch):
    from threading import Event

    captured = {}
    control = WebControl(lambda _: 0, logging.getLogger("test"))
    plan = mock.Mock(abort_requested=Event())
    finished = Event()

    def run():
        plan.abort_requested.wait(5)
        finished.set()
        raise SyncAborted("Upload aborted")
    ...
    client = captured["app"].test_client()
    headers = {"X-CSRF-Token": client.get("/auth-state").json["csrf_token"]}
    assert control._start_task(run, active_frame=("current", plan))[0]
    try:
        assert client.post("/abort-frame-sync", json={"token": "current"}).status_code == 403
        assert (
            client.post(
                "/abort-frame-sync", json={"token": "previous"}, headers=headers
            ).status_code
            == 409
        )
        assert not plan.abort_requested.is_set()
        assert (
            client.post("/abort-frame-sync", json={"token": "current"}, headers=headers).status_code
            == 202
        )
        ...
        for _ in range(100):
            ...
            if not state["running"]:
                break
            time.sleep(0.01)
        assert state["progress"]["phase"] == "aborted"
        assert state["last_exit_code"] == 130
...
def test_unresolved_photo_is_reviewed_and_can_be_left_out_without_erasing_history(tmp_path):
    ...
    from icloudpd.frame_sync import UploadLedger
