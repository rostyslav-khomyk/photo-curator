...
from icloudpd.google_photos_client import GooglePhotosClient, GoogleRateLimited
...
def test_retry_after_http_date(monkeypatch):
    monkeypatch.setattr("icloudpd.google_photos_client.time.time", lambda: 0)
...
def test_wait_publishes_countdown_and_checks_abort(monkeypatch):
    ...
    monkeypatch.setattr("icloudpd.google_photos_client.time.monotonic", lambda: clock[0])
    monkeypatch.setattr(
        "icloudpd.google_photos_client.time.sleep",
