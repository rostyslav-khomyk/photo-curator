...
- Removed the inherited iCloud downloader, web dashboard, Docker/npm distribution,
  downloader fixtures, and duplicate proof-of-concept files.
...
5. **Testing, Build & Verification:**
   ...
   - Single release build via `Scripts/build_app.sh`, code signature verified, and relaunched at PID 49623.
...
3. **Automated Testing & Single Release Build:**
   ...
   - Single release build executed via `Scripts/build_app.sh`, verified code signature, and relaunched app at PID 48050.
...
4. **Automated Testing & Single Release Build:**
   ...
   - Single batched release build executed via `Scripts/build_app.sh`; verified codesign and relaunched app running at PID 46782.
...
5. **Verification & Single Release Build:**
   ...
   - Single release build executed via `Scripts/build_app.sh` (`dist/Photo Curator.app`); strict deep code signature verified.
...
Single batched release build completed via Scripts/build_app.sh (Swift release build 29.07s,
...
Scripts/build_app.sh (Swift release build 30.59s, engine packaging, icon synthesis); deep and
...
Scripts/build_app.sh; deep/strict codesign verification passed (ad-hoc fallback).
...
Verification: `swift test`: 19 passed (6 new queue tests).
...
explicitly unverified. Repeatable probe: `swift Tools/check_curator_capabilities.swift`.
...
- Added reproducible vector app artwork (`Scripts/render_icon.swift`), all standard iconset
